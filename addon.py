...
def add_video_items(videoitems):
    for video in videoitems:
        ...
        url = get_url(action='play', whatson_id=video['whatson_id'], video_url= video['video']['link'])
        is_folder = False
        xbmcplugin.addDirectoryItem(_handle, url, list_item, is_folder)


def play_video(whatson_id, video_url):
    # subtitle = uzg.get_ondertitel(whatson_id)
    stream_info = uzg.get_play_url(whatson_id, video_url)
    playitem = xbmcgui.ListItem(path=stream_info['stream']['streamURL'])

    if (stream_info['assets'].get('subtitles') != None and xbmcplugin.getSetting(_handle, "subtitle") == 'true'):
        subtitle = stream_info['assets']['subtitles'][0]; 
        playitem.setSubtitles([subtitle['location']])
    ...
    playitem.setProperty('inputstream.adaptive.license_key', stream_info['stream']['drmToken'])
...
def router(paramstring):
    ...
    if params:
        ...
        if params['action'] == 'keuze':
            ...
            play_video(params['whatson_id'], params['video_url'])
        else:

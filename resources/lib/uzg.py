...
class Uzg:
    ...
    def getChannels(self):
        return Channels('https://npo.nl/start/api/domain/guide-channels').uzgitemlist
    ...
    def getQueryPage(self, tekst):
        ...
        return SerieItems("https://npo.nl/start/api/domain/search-results?query={}&searchType=series&subscriptionType=anonymous".format(tekst.replace(' ', '%20'))).uzgitemlist

    def episodesOrseason(self, url):
        if ('programs-by-season' in url):
            # we hebben te maken met afleveringen onder een season, deze kunnen gelijk terug.
            return EpisodesItems(url=url).get_episodes_info_and_items();
        # if ('series-seasons' in url ):
        #   # we hebben te maken met seizoenen.
        #   return EpisodesItems(url=url).get_episodes_info_and_items();
        ...
        return {'type': 'season', 'items': SeasonItems(data).uzgitemlist}          
    ...
    def get_play_url(self, whatson_id, video_url):
        return self.npoHelpers.get_play_url(whatson_id, video_url)
